...
def test(model,device, dataloader, loss_fn, loss_fn_MAE, return_pred=False):
    ...
    return (test_loss, test_loss_MAE, pred) if return_pred else (test_loss, test_loss_MAE)
...
def main():
    ...
    #save path
    file_savepath =f"./output/GNN_TL_{args.model_name}_seed_{args.seed}"
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
        ...
        test_dataloader = GraphDataLoader(dataset, batch_size=len(test_subsampler), sampler = test_subsampler)

        log_file, best_model = train_with_es(model, device, train_dataloader, test_dataloader, lr, loss_fn, loss_MAE, epochs=epoch, early_stop=30, )
        save_log_file(best_model, log_file, file_savepath, fold)
...
def train_with_es(model, device, train_dataloader, test_dataloader, lr, loss_fn, loss_MAE, epochs=200, early_stop=30,):
    ...
    return (log_file, best_model)


def save_log_file(best_model, log_file, file_savepath, fold):
    ...
    torch.save(best_model.state_dict(), os.path.join(file_savepath, f"fold_{fold}_best_model_weight.pth"))


def return_prediction(model, device, file_savepath, loss_fn, loss_MAE, fold, dataloader, y_labels):
    best_path = os.path.join(file_savepath, f"fold_{fold}_best_model_weight.pth")

    best_model = model.to(device)
    checkpoint = torch.load(best_path, map_location=device)  # 加载断点
    best_model.load_state_dict(checkpoint)  # 加载模型可学习参数

    _, _, pred = test(model, device, dataloader, loss_fn, loss_MAE, return_pred=True)
    y = y_labels
    y = y.reshape(-1, 1).cpu()
